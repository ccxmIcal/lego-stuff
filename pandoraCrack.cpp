...
__declspec(naked) void rebase_interface_pointers()
{
	__asm
	{
		...
		push ebp // fixing addreses
		push eax // push arg
		call fix_interface_address // call
		pop ebp // get saved ebp
		mov[ebp - 0x40], eax // replacing correct addresses
		POPAD // get all original registers
		push g_interface_jmp_back
		ret
	}
}
...
char fix_username()
{
	static const char* name = "sufi_rawr   ";
...
void main()
{
	...
	*reinterpret_cast<bool*>(0x40FCC47B) = 0;
	...
	printf("[+] finished!\n");
	printf("[+] Skeet crack renamed by sufi");
}
